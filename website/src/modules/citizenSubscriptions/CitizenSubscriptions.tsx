...
const CitizenSubscriptions: FC<Props> = ({
    ...
    if (citizenId) {
      await getCitizenName(citizenId)
        .then((result: Citizen): void =>
          setCitizenName(
            `${result.lastName.toUpperCase()} ${capitalize(result.firstName)}`
          )
        )
        .catch((err: any) => {
          navigate('/');
  ...
  useEffect(() => {
    if (location?.state?.lastName) {
      setCitizenName(
        `${location?.state?.lastName.toUpperCase()} ${capitalize(
  ...
  useEffect(() => {
    if (breadCrumbData && breadCrumbData.length < 3) {
      const breadCrumb = { ...location, crumbLabel: Strings['dashboard.management.employees.subscriptions.title'] };
