...
const CitizenSubscriptionCard: FC<CitizenSubscriptionCardProps> = ({
  ...
  const renewSubscription = async () => {
    ...
    try {
      ...
      navigate(`/subscriptions/new?incentiveId=${incentiveId}`);
    } catch {}
  };
  ...
  switch (status) {
    ...
    case STATUS.VALIDATED:
      ...
      if (amount) {
        ...
        if (!!lastPayment) {
          infos += `${
            Strings['dashboard.citizen.subscriptions.subscription.date.until']
          } ${format(new Date(lastPayment!), 'dd/MM/yyy')}`;
        }
      }
        ...
        <div className="status-description">
          {`${
            status === STATUS.TO_PROCESS
              ? Strings['dashboard.date.requested.on']
              : Strings['dashboard.date.the']
          } ${formatedDate}`}
        </div>
        <LinesEllipsis
