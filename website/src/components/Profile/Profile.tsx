...
import { setCompaniesList, formattedDateFile } from '@utils/helpers';
...
const Profile: FC<ProfileProps> = ({ crumbs }) => {
  ...
  const sanitizeCitizenData = async (citizen: Citizen) => {
    let sanitizedCitizenData: User = { ...citizen };
    const consentsData = await getConsentsById(citizen.id);
    const enterprisesList = await getEntreprisesList<EntrepriseName[]>();
    const company = enterprisesList.find(
      (item: { id: string }) => item.id === citizen.affiliation.enterpriseId
    );
    const companies: object[] = setCompaniesList(enterprisesList);
    setCompanyOptions(companies);
    sanitizedCitizenData.statusPhrase =
      UserStatus[citizen.status as keyof typeof UserStatus];
  ...
  const sanitizeUserFunderData = async (userFunder: UserFunder) => {
    const { communityIds } = userFunder;
    let sanitizedUserFunderData: User = { ...userFunder };
    ...
    if (communityIds) {
      const userFunderCommunities = await getUserFunderCommunities(userFunder);
