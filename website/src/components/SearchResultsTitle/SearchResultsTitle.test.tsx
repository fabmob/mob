...
describe('<SearchResultsTitle />', () => {
  ...
  test('No search with zero result', () => {
    const { container, getByText } = render(
      <SearchResultsTitle nbResult={0} />
    );
    expect(
      container.getElementsByClassName('mcm-search-results-title').length
    ).toBe(1);
    expect(getByText('Tous les résultats (0)')).toBeInTheDocument();
  });
