...
describe('OfferCard Component', () => {
  test('renders totalValue = 1 and one offer card', async () => {
    const { getByText, container } = render(<OfferCard dataList={mockedData} />);

    await waitFor(() => {
      expect(getByText("Bonus Ecologique pour l'achat d'un vélo électrique")).toBeInTheDocument();
      expect(getByText("1 citoyen sur 1 ayant validé une demande bénéficie de l'aide")).toBeInTheDocument();
