...
export class S3Service extends S3Config {
  ...
  private ALLOWED_FILE_SIZE = 5000000;
  ...
  async deleteObjectFile(bucketName: string, filePath: string): Promise<any> {
    const listParams = {
      Bucket: bucketName,
      Prefix: filePath,
    };
    ...
    justifObjects.Contents?.forEach((content: any) => {
   ...
   * @param fileList Express.Multer.File
   */
  hasCorrectNumberOfFiles(fileList: any): Boolean {
