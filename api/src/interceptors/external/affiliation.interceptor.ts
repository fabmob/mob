...
import {FunderService, IUser} from '../../services';
import {
  FUNDER_TYPE,
  INCENTIVE_TYPE,
...
export class AffiliationInterceptor implements Provider<Interceptor> {
  ...
  async intercept(
    ...
    if (!roles?.includes(Roles.MAAS_BACKEND)) {
      ...
      const funders = inputFunderId && (await this.funderService.getFunders());
      const funderMatch = funders && funders.find(({id}) => inputFunderId === id);
