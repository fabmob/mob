...
function authenticate(context) {
  ...
  var citizenGroup = new ArrayList();
  // Search citizen group
  citizenGroup = realm.searchForGroupByName('citoyens', null, null)

  // Add lastLoginAt for citizen users
  if (citizenGroup.length && user.isMemberOf(citizenGroup[0])) {
    //Set lastLoginAt timestamp attribute
    var lastLoginAt = new ArrayList();
    lastLoginAt.add(JSON.stringify(Date.now()));
    user.setAttribute("lastLoginAt", lastLoginAt);

    // Delete boolean notification if user connects
    if(user.getAttribute("isInactivityNotificationSent").length) {
      user.removeAttribute("isInactivityNotificationSent")
    }
  }
